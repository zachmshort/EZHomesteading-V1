...
export default function ItemPath({ selectedCategoryIndex, selectedSubcategoryIndex, selectedItemIndex, items }) {
  ...
  const renderCategoryView = () => {
    if (
      ...
      const selectedSubcategory = categorySubcategories[selectedCategory]
        ...
        : null;
      const selectedItem =
        selectedSubcategory && items[selectedCategoryIndex] && items[selectedCategoryIndex][selectedSubcategoryIndex]
          ? items[selectedCategoryIndex][selectedSubcategoryIndex][selectedItemIndex]
          : null;
      if (selectedCategory && selectedSubcategory && selectedItem) {
        return (
          <div>
            <h4 className="text-4xl tracking-tight text-gray-900 sm:text-6xl">
              {selectedCategory} - {selectedSubcategory} - {selectedItem} {/* Display full path */}
            </h4>
          </div>
        );
      }
        ...
        <div>
          {categories.map((category, categoryIndex) => (
            <div key={categoryIndex}>
              <h4 className="text-4xl tracking-tight text-gray-900 sm:text-6xl">{category}</h4>
              {categorySubcategories[category].map((subcategory, subcategoryIndex) => (
