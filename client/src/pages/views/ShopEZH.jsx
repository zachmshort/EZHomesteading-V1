...
export default function ShopEZH() {
  ...
  return (
    <>
              ...
              <Popover.Panel className="fixed pb-5 inset-x-0 top-0 z-50 bg-white pt-4 shadow-lg ring-1 ring-gray-900/5">
                <div className="mx-auto grid max-w-7xl grid-cols-6 gap-2 px-4 sm:grid-cols-6 sm:gap-2 lg:gap-2 lg:px-4 xl:gap-2">
                  {Object.keys(categoryProducts).map((categoryName) => (
                    <div
                      key={categoryName}
                      className={`group relative flex flex-col items-center gap-2 rounded-lg p-2 text-xs leading-5 hover:bg-gray-50 ${selectedCategory === categoryName ? 'bg-gray-200' : ''}`}
      ...
      <DropDownButtons />
      <div className="grid grid-cols-3 gap-4 mt-4 right-10 px-4 w-3/4 flex">
        {selectedCategory && categoryProducts[selectedCategory].map(product => (
          <ProductCard
